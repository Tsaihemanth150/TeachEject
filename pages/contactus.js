...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/users/contact`, {
        method: "POST",
        headers: {
...
India
    ...
    />
                  <button
                    type="submit"
                    className="w-full p-3 bg-teal-800 text-white transition border rounded border-primary bg-primary hover:bg-opacity-90"
                  >
                    Send Message
                  </button>
                </div>
